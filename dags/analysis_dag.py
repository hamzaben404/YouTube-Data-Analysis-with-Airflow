...
def generate_visualizations(**kwargs):
    ...
    files = [
        f for f in os.listdir(PROCESSED_DIR) if f.startswith("video_data_processed")
    ]
    if not files:
        raise FileNotFoundError("No processed data file found.")

    latest_file = max(
        files, key=lambda f: os.path.getctime(os.path.join(PROCESSED_DIR, f))
    )
...
def analyze_sentiments(**kwargs):
    import pandas as pd
    from transformers import pipeline
    processed_file_path = kwargs["dag_run"].conf.get("processed_file_path")

    if not processed_file_path:
        raise FileNotFoundError("Processed file path not found in dag_run.conf.")

    video_df = pd.read_csv(processed_file_path)
    # Perform sentiment analysis
    video_df["descriptionSentiment"] = video_df["description"].apply(
        lambda desc: (
            sentiment_analyzer(desc[:512])[0]["label"]
            if pd.notnull(desc)
            else "NEUTRAL"
        )
    )
